        ...
        <div v-else>
          <el-form-item v-if="ruleForm.shipin" label="视频" prop="shipin">
            <el-button type="text" size="small" @click="download(ruleForm.shipin)">预览</el-button>
                                                                  ...
                                                                  <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="发日期" prop="fariqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.fariqi" 
                type="date"
                placeholder="发日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.fariqi" label="发日期" prop="fariqi">
              <el-input v-model="ruleForm.fariqi" 
                placeholder="发日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                              </el-row>
                                                                                                                                                                          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="技巧内容" prop="jiqiaoneirong">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.jiqiaoneirong" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.jiqiaoneirong" label="技巧内容" prop="jiqiaoneirong">
                    <span v-html="ruleForm.jiqiaoneirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shipin = true;
            continue;
	            	            ...
	            	            if(o=='jiqiaoneirong'){
            this.ruleForm.jiqiaoneirong = obj[o];
	    this.ro.jiqiaoneirong = true;
            continue;
          }
	            	            if(o=='fariqi'){
            this.ruleForm.fariqi = obj[o];
	    this.ro.fariqi = true;
            continue;
          }
	                    }
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `nongyejiqiao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.nongyejiqiaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
